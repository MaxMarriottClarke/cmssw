    ...
    for (int start = 0; start < total; start += mb) {
      ...
      for (int bi = 0; bi < n; ++bi) {
        ...
        for (int k : clusterIndices) {
          ...
          const int j = rhtools.getLayerWithOffset(clusters.indexes()[v].seedID()) - 1;
          ...
          inputTensor[base + 0] = static_cast<float>(clusters.energy()[v].energy() / static_cast<float>(ts.vertex_multiplicity(k)));
          inputTensor[base + 1] = static_cast<float>(std::abs(clusters.eta(v)));
          inputTensor[base + 2] = static_cast<float>(clusters.phi(v));
