    ...
    for (int start = 0; start < total; start += mb) {
      ...
      for (int bi = 0; bi < n; ++bi) {
        ...
        for (int k : clusterIndices) {
          ...
          inputTensor[base + 0] = clusters.energy()[v].energy() / static_cast<float>(ts.vertex_multiplicity(k));
          inputTensor[base + 1] = std::abs(clusters.eta(v));
          inputTensor[base + 2] = clusters.phi(v);
